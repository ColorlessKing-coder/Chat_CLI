...
class Listener:
    ...
    def handle_client(self, connection, address):
        try:
            ...
            self.clients[
                connection] = nickname  # Gelen Bağlantıya Kullanıcıyı Atadık, Bağlantıyı nickname Olarak Tutuyoruz
            ...
            panel1 = Panel(
                f"[green]{nickname} : [white]Connected From[/white] [blink]{address[0]}:{address[1]}[/blink] [/green]",
                title="", subtitle="", border_style="white")
            ...
            while True:
                    ...
                    try:
                        ...
                        for connect, user_name in self.clients.items():
                            if user_name == user_info:  # Bağlantıyı kullanıcı adına göre bul
                                ...
                                connection.send(
                                    f"TAKE_USERS {user_list.decode()}".encode())  # İlgili bağlantıya kullanıcı listesini gönder
                    except ValueError:
                ...
                if response_message.startswith("RECEIVER_MESSAGE_NAME"):
                    try:
                        _, user_name, content = response_message.split(
                            maxsplit=2)  # Dikkat user_name Mesajı Kime Göndereceğini Temsil Ediyor
                        ...
                        for connect, user in self.clients.items():
                            if self.clients[connect] == user_name:
                                connect.send(f'TAKE_SPECIAL_MESSAGE {full_message}'.encode(
                                    self.encode))  # kullanarak, metni UTF-8 formatında bytes türüne dönüştürdük.
    ...
    def listen_for_connections(self, ip_address, port):
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                ...
                sock.listen()
                panel1 = Panel("[blink][white]Server is listening for connections...[/white][/blink]", title="",
